...
## Тестирование API
1. Создание нового воркфлоу.
   Для старта процесса оформления заказа выполните следующий запрос:
```bash
...
```
4. Запуск воркфлоу с эмитацией ошибки на степе inventory.
Вызов ошибки воркфлоу реализован случайным образом, но для демонстрации функционала был реализован вызов ошибки через параметры

__"error": "error"__, (не использовал bool значения, так как для будущего тестирования могут понадобятся другие типы ошибок), 

__"step": "inventory"__ 
...
5. Как выглядит успешный воркфлоу с ретраем:
![img_3.png](images/img_3.png)
Видим что степ order выполнялся два раза, при этом второй раз был выполнен успешно и это не привело к вызову компенсационного воркфлоу
